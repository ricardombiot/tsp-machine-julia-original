...
function test_bench()
   ...
   for n in 7:10
      ...
      weight = Weight(25)
      graf = GrafGenerator.completo(n_color, weight)
      b = @benchmarkable test_grafo_with_g(x, $weight, false) setup=(x = deepcopy($graf)) samples=1 seconds=1000.0
      println("#### Time K$n")
      println(run(b))
   end
...
function test_grafo_with_g(graf, weight, log = false)
   path = "./data/disk"
